...
export async function PATCH(request: Request, { params }: Props) {
  try {
    ...
    const existing = await prisma.surveyAppointment.findFirst({ where: { id: appointmentId, organizationId: membership.organizationId } });
    ...
    await prisma.surveyAppointment.update({ where: { id: existing.id }, data: { customerId, projectId: body?.projectId?.trim() || null, assignedToId: body?.assignedToId?.trim() || null, title, location, contactName: body?.contactName?.trim() || null, contactPhone: body?.contactPhone?.trim() || null, scheduledStart: new Date(scheduledStart), scheduledEnd: body?.scheduledEnd ? new Date(body.scheduledEnd) : null, status: parseStatus(body?.status), note: body?.note?.trim() || null } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Update survey appointment error:", error);
...
export async function DELETE(_request: Request, { params }: Props) {
  try {
    ...
    const existing = await prisma.surveyAppointment.findFirst({ where: { id: appointmentId, organizationId: membership.organizationId } });
    if (!existing) return NextResponse.json({ error: "Appointment not found" }, { status: 404 });
    await prisma.surveyAppointment.delete({ where: { id: existing.id } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Delete survey appointment error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
